...
run_exact() {
  ...
  case "$target_kind" in
    --lib)
      list_output=$("$cargo_bin" test --locked --manifest-path orch/Cargo.toml -p "$package" --features selfhost --lib "$test_name" -- --list --ignored --exact)
      ;;
    --test)
      list_output=$("$cargo_bin" test --locked --manifest-path orch/Cargo.toml -p "$package" --features selfhost --test "$target_name" "$test_name" -- --list --ignored --exact)
      ;;
    *)
  ...
  case "$target_kind" in
    --lib)
      "$cargo_bin" test --locked --manifest-path orch/Cargo.toml -p "$package" --features selfhost --lib "$test_name" -- --ignored --exact --test-threads=1
      ;;
    --test)
      "$cargo_bin" test --locked --manifest-path orch/Cargo.toml -p "$package" --features selfhost --test "$target_name" "$test_name" -- --ignored --exact --test-threads=1
      ;;
  esac
}
