...
#[allow(dead_code)] // sibling test still exercises the legacy-only selector
...
fn stale_repo_fixture(name: &str, materialize_build_tree: bool) -> Option<StaleRepo> {
    ...
    // remains suitable for a locally owned recovery fixture.
...
fn prepare_stale_snapshot_probe(
    name: &str,
    committed_build_binding_drift: bool,
) -> Option<(StaleRepo, String)> {
    let mut repo = stale_repo_fixture(name, true)?;
    if committed_build_binding_drift {
        drift_project_binding(&repo, "# B287 committed build-tree binding drift\n");
        commit_tracked_scratch_state(&mut repo, "B287 committed binding drift fixture");
    }
...
fn drift_project_binding(repo: &StaleRepo, marker: &str) {
    let binding_path = repo.root.join("coordination/PROJECT-BINDING.yaml");
    let mut binding = fs::read_to_string(&binding_path).expect("读取 scratch binding 失败");
    binding.push('\n');
    binding.push_str(marker);
    fs::write(&binding_path, binding).expect("写 scratch binding drift 失败");
}
...
fn repair_materialized_round_contract(repo: &StaleRepo, round: &str) {
    ...
    let planned = orch_host::plan::run_plan(&repo.root)
        .expect("B287 production schema-3 planner 必须修复继承轮态");
    ...
    orch_host::ledger::append(&repo.root, round, &[signed]).unwrap();

    let control = run_orch(&repo.root, &["snapshot"]);
    assert_stale_read_allowed(repo, &control, "snapshot self-consistent control");
}

fn prepare_recovery_fixture(root: &Path) -> (PathBuf, Vec<u8>) {
    let ledger = root.join("coordination/rounds/r-stale-probe/events.jsonl");
    let wal = root.join("coordination/runtime/ledger-wal/r-stale-probe.jsonl");
    fs::create_dir_all(ledger.parent().unwrap()).unwrap();
    fs::create_dir_all(wal.parent().unwrap()).unwrap();
    let event = b"{\"eventId\":\"stale-probe-1\",\"ts\":\"2026-08-20T00:00:00Z\",\"actor\":\"runtime:orch\",\"type\":\"RoundOpened\",\"round\":\"r-stale-probe\"}\n";
    fs::write(&ledger, b"").unwrap();
    fs::write(wal, event).unwrap();
    (ledger, event.to_vec())
}

fn assert_round_independent_stale_read(repo: &StaleRepo, scenario: &str) {
    let (ledger, _) = prepare_recovery_fixture(&repo.root);
    let recovery = run_orch(&repo.root, &["ledger", "recover", "--round", "r-stale-probe"]);
    assert_stale_read_allowed(repo, &recovery, &format!("ledger recover dry-run ({scenario})"));
    assert!(fs::read(ledger).unwrap().is_empty(), "read-only recovery wrote ledger");
}
...
fn stale_state_change_is_rejected_and_override_is_audited() {
    ...
    let (ledger, expected) = prepare_recovery_fixture(&repo.root);
    let denied = run_orch(&repo.root, &["ledger", "recover", "--round", "r-stale-probe", "--apply"]);
    assert_stale_write_rejected(&repo, &denied, "ledger recover --apply");
    assert!(fs::read(&ledger).unwrap().is_empty(), "refusal must precede durable ledger write");

    let allowed = run_orch(&repo.root, &["--allow-stale-binary", "ledger", "recover", "--round", "r-stale-probe", "--apply"]);
    assert_stale_override_audited(&repo, &allowed, "ledger recover --apply");
    assert_eq!(fs::read(&ledger).unwrap(), expected, "override must perform the actual recovery");
}
...
fn stale_read_only_doctor_remains_available_and_warns() {
    ...
    // Author the minimum historical doctor fixture directly; the retired init
    // writer is not part of the staleness contract.
    use std::os::unix::fs::PermissionsExt;
    let (ledger, expected) = prepare_recovery_fixture(&repo.root);
    fs::write(ledger, expected).unwrap();
    fs::write(repo.root.join(".gitignore"), "coordination/rounds/*/dispatch/\ncoordination/runtime/\n.worktrees/\n").unwrap();
    let wait = repo.root.join("coordination/scripts/wait-dispatch.sh");
    fs::create_dir_all(wait.parent().unwrap()).unwrap();
    fs::write(&wait, "#!/bin/sh\nexit 0\n").unwrap();
    fs::set_permissions(wait, fs::Permissions::from_mode(0o755)).unwrap();
...
fn stale_snapshot_distinguishes_read_from_write() {
    ...
    assert!(
        !overridden.status.success()
            ...
            && overridden_stderr.contains("PlanSignedOff"),
        "逃生舱只绕过 stale guard，不得绕过 v3 sign-off 边界: {overridden_stderr}"
    );
    assert!(!snapshot_path.exists(), "轮态校验失败时不得写 snapshot");
}

#[test]
fn stale_snapshot_stays_read_only_after_registry_retirement_and_binding_drift() {
    ...
    drift_project_binding(&repo, "# B287 committed T2 binding drift probe\n");
    commit_tracked_scratch_state(&mut repo, "B287 committed T2 binding drift");

    // T2: an actual signed source moved beyond the IR after tracked registry
    // retirement. The stale guard remains independent from round validation.
    assert_round_independent_stale_read(&repo, "binding drift");
...
fn missing_git_metadata_degrades_with_an_explanation() {
    ...
    let output = fixture_orch_command(&[])
        .arg("--root")
        .arg(&root)
        .args(["ledger", "recover", "--round", "r-stale-probe"])
        .env(
