...
mod schedule_cli_tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        let dir = worktree_orch_dir()
            .join("target")
            .join("test-tmp")
            .join(format!("b135-schedule-{name}-{}", std::process::id()));
        if dir.exists() {
    ...
    fn dispatch(aid: &str, no: u64, agent: &str, ts: &str) -> String {
        format!(
            r#"{{"eventId":"evt-dispatch-{no}","ts":"{ts}","actor":"runtime:orch","type":"DispatchIssued","taskId":"BSCH","round":"{ROUND}","payload":{{"agent":"{agent}","attemptId":"{aid}","attemptNo":{no},"goPath":"coordination/rounds/{ROUND}/dispatch/{agent}/GO-{aid}.md","baseSha":"0123456789abcdef0123456789abcdef01234567"}}}}"#
        )
    }

    fn crashed(aid: &str, no: u64, ts: &str) -> String {
        format!(
            r#"{{"eventId":"evt-crash-{no}","ts":"{ts}","actor":"runtime:orch","type":"AttemptCrashed","taskId":"BSCH","round":"{ROUND}","payload":{{"attemptId":"{aid}","attemptNo":{no},"lastSignal":"heartbeat-expired"}}}}"#
        )
    }

    fn write_fixture(root: &std::path::Path, events: &[String]) {
        ...
        .unwrap();
        fs::write(
            root.join(format!("coordination/rounds/{ROUND}/tasks/BSCH.md")),
    ...
    fn fingerprint(root: &std::path::Path) -> BTreeMap<String, (u64, u64)> {
        fn fnv(bytes: &[u8]) -> u64 {
            ...
            for entry in fs::read_dir(&dir).unwrap() {
                ...
                if path.is_dir() {
                    ...
                    let rel = path
                        ...
                        .to_string_lossy()
                        .into_owned();
                    map.insert(rel, (bytes.len() as u64, fnv(&bytes)));
                }
            }
        }
        map
    ...
    fn run_schedule(root: &std::path::Path) -> std::process::Output {
    ...
    #[test]
    fn schedule_cli_one_crash_keeps_candidate_and_is_readonly() {
        let root = temp_root("one-crash");
        write_fixture(
            &root,
            &[
                dispatch("BSCH-A0001", 1, "executor-claw", "2026-07-27T04:00:00Z"),
                crashed("BSCH-A0001", 1, "2026-07-27T05:00:00Z"),
            ],
        );
        let before = fingerprint(&root);
        let output = run_schedule(&root);
        assert!(
            output.status.success(),
            "stderr: {}",
            String::from_utf8_lossy(&output.stderr)
        ...
        assert!(
            stdout.contains("strikes: executor-claw=1"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("failed (两振出局): none"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("next candidate: executor-claw"),
            "stdout: {stdout}"
        );
        assert!(
            stdout
                .contains("current attempt: BSCH-A0001 agent=executor-claw status=AttemptCrashed"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("escalation chain: executor-claw -> executor-opencode"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("review chain: primary=executor-opencode"),
            "stdout: {stdout}"
        );
        assert_eq!(
            before,
            fingerprint(&root),
            "只读 schedule 命令不得改变仓库任何字节"
        );
        fs::remove_dir_all(&root).unwrap();
    }

    #[test]
    fn schedule_cli_two_strikes_escalates_down_the_chain() {
        let root = temp_root("two-strikes");
        write_fixture(
            &root,
            &[
                dispatch("BSCH-A0001", 1, "executor-claw", "2026-07-27T04:00:00Z"),
                crashed("BSCH-A0001", 1, "2026-07-27T05:00:00Z"),
                dispatch("BSCH-A0002", 2, "executor-claw", "2026-07-27T06:00:00Z"),
                crashed("BSCH-A0002", 2, "2026-07-27T07:00:00Z"),
            ],
        );
        let output = run_schedule(&root);
        assert!(
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            stdout.contains("strikes: executor-claw=2"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("failed (两振出局): executor-claw"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("next candidate: executor-opencode"),
            "stdout: {stdout}"
        );
        assert!(
            stdout
                .contains("previous attempt: BSCH-A0001 agent=executor-claw status=AttemptCrashed"),
            "stdout: {stdout}"
        );
        assert!(
            stdout
                .contains("current attempt: BSCH-A0002 agent=executor-claw status=AttemptCrashed"),
            "stdout: {stdout}"
        );
        assert!(
            stdout.contains("review chain: primary=executor-opencode"),
