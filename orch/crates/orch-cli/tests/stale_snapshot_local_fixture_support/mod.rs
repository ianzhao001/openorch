...
fn legacy_contract(events: &str, path: &Path) -> Result<bool, String> {
    let mut opened = None;
    for (index, line) in events
        .lines()
        .filter(|line| !line.trim().is_empty())
        .enumerate()
    {
        ...
        if event.get("type").and_then(Value::as_str) != Some("RoundOpened") {
            continue;
        }
        if opened.is_some() {
            return Err(format!(
                "materialized ledger {} 含多条 RoundOpened",
                path.display()
            ));
        }
        let version = match event.pointer("/payload/contractSchemaVersion") {
            None => None,
            Some(value) => Some(value.as_u64().ok_or_else(|| {
                format!(
                    "materialized ledger {} 的 contractSchemaVersion 非整数",
                    path.display()
                )
            })?),
        };
    ...
    match opened.flatten() {
        None | Some(1 | 2) => Ok(true),
        Some(3) => Ok(false),
        Some(version) => Err(format!(
            "materialized ledger {} 的 contractSchemaVersion={version} 未建模",
            path.display()
        )),
    }
}
...
fn prepare_local_valid_materialized_round_inner(
    root: &Path,
    legacy_only: bool,
    ...
    for entry in entries {
        ...
        if validated && (!legacy_only || legacy_contract(&events, &events_path)?) {
    ...
    let (_, round, events_path, events, closed) = candidates.pop().ok_or_else(|| {
        format!(
            "调用方 materialized tree {} 中找不到带 ROUND-IR 与 runtime TaskValidated 的{}轮模板",
            root.display(),
            if legacy_only { " schema1/2" } else { "" },
        )
    })?;
...
#[allow(dead_code)] // shared module: the legacy consumer intentionally uses the sibling selector
pub fn prepare_local_valid_materialized_round(root: &Path) -> Result<String, String> {
    prepare_local_valid_materialized_round_inner(root, false)
}

pub fn prepare_local_valid_materialized_legacy_round(root: &Path) -> Result<String, String> {
    prepare_local_valid_materialized_round_inner(root, true)
}
