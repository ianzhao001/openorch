...
#[test]
fn retired_wave_and_legacy_verify_reject_model_override() {
    let root = root();
    let output = fixture_orch_command(&[])
        .args([
            "--root",
    ...
    assert!(String::from_utf8_lossy(&verify.stderr).contains("unrecognized subcommand"));
}
...
fn retired_step_serve_and_wave_never_spawn_external_provider() {
    for (label, args) in [
        ("step", vec!["step"]),
        ("serve", vec!["serve", "--once"]),
        ("wave", vec!["run-wave", "--timeout-secs", "1"]),
    ] {
        let (root, sentinel) = signed_collected_root();
        // The retired inbox producer is not needed to place adversarial input.
        fs::create_dir_all(root.join("coordination/inbox")).unwrap();
        fs::write(root.join("coordination/inbox/tempt-planner.md"), format!("tempt planner from {label}")).unwrap();
        let output = fixture_orch_command(&[])
            .arg("--root")
            .arg(&root)
            .args(&args)
