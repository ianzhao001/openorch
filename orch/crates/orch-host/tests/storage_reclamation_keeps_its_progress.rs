...
fn trigger_at() -> usize {
    find(CLOSE_BYTES, TRIGGER_FN)
}
fn trigger_end() -> usize {
    find_from(CLOSE_BYTES, ITEM_END, trigger_at())
}
fn cleanup_at() -> usize {
    find(CLI_BYTES, CLOSE_CLEANUP_FN)
}
fn cleanup_end() -> usize {
    find_from(CLI_BYTES, ITEM_END, cleanup_at())
}
...
fn frozen_source_contract_05() {
    ...
    assert!(std::process::Command::new("git")
        ...
        .unwrap()
        .success());
    // Recorded historical source stays immutable; this current target now checks
    // durable maintenance feedback instead of a removed call-name/window.
    fs::create_dir_all(root.join("coordination/rounds/rClosed")).unwrap();
    let ledger = root.join("coordination/rounds/rClosed/events.jsonl");
    let history=b"{\"eventId\":\"closed\",\"ts\":\"2026-09-08T00:00:00Z\",\"actor\":\"runtime:orch\",\"type\":\"RoundClosed\",\"round\":\"rClosed\",\"payload\":{}}\n";
    fs::write(&ledger, history).unwrap();
    let report = orch_host::reclaim::maintain_storage(&root, false).unwrap();
    assert_eq!(fs::read(&ledger).unwrap(), history);
    let saved = orch_host::reclaim::latest_maintenance_report(&root)
        .unwrap()
        .unwrap();
    assert_eq!(saved.removed_logical_bytes, report.removed_logical_bytes);
    assert_eq!(saved.items.len(), report.items.len());
...
fn scratch_root(name: &str) -> PathBuf {
    ...
    orch_root
        ...
        .join(format!("b263-{name}-{}-{seq}", std::process::id()))
}
...
fn a_scratch_sweep_reports_everything_it_removed_even_when_one_entry_refuses() {
    ...
    let report: ScratchSweepReport = sweep_test_scratch_root(&root, Duration::ZERO, &[])
        .expect("a sweep that reclaimed real entries must not present itself as a total failure");
...
fn a_reap_refusal_is_structured_evidence() {
    let report: SiteReapReport =
        reap_released_sites_reported(Path::new("/nonexistent-b263"), "r90").unwrap_or_else(
            |error| panic!("a missing root is not a reason to lose the report: {error:#}"),
        );
