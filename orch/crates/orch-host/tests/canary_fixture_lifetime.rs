...
//!
//! B323 schema3 evolution: historical B194 source/event/commit remain immutable.
//! The three real settle-budget laws below remain unchanged. The retired B143
//! live caller must not be recreated merely to satisfy its old source sentinel;
//! the current exclusive H56 contract is pure historical decision/codec coverage.
...
/// The entire old daemon/pure-sampling carrier retires; process lifetime checks above remain.
#[test]
fn retired_serve_module_has_no_remaining_canary() {
    assert!(!Path::new(env!("CARGO_MANIFEST_DIR")).join("src/serve.rs").exists());
}
