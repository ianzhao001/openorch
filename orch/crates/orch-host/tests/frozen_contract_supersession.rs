...
//!    **不扫本轮 `seeds/`**，`check-frozen-prefix-conflicts.sh:52`），下轮再「合法地」以
...
//! M7. **接线变异**：谓词存在但落位校验路径不调用置换检查
//!     -> `the_seed_path_validation_consults_the_landed_digest` 红。
//! M8. 把闸修成全拦：合法置换也被拒
...
// 刻意**程序化构造**这三个 64 位十六进制夹具，而不是写成字面量：
// 本文件 `include_str!` 了一个生产文件，若同时出现 `"[0-9a-f]{64}"` 字面量，
// 就会被 `check-frozen-prefix-conflicts.sh` 的启发式当成「整段哈希钉死」候选
// —— 而那恰恰是本轮 Q1 裁决点名要收紧的假阳性来源（pi ⑤ / claw ⑤）。
// 立这张卡的人，先别让自己成为下一个样本。
...
// ── 接线变异（M7）：落位校验路径必须真的核对落位摘要 ──────────────────────

#[test]
fn the_seed_path_validation_consults_the_landed_digest() {
    let anchor = "pub fn validate_seed_paths(";
    let start = ORACLE_SRC
        .find(anchor)
        .expect("B270: missing anchor validate_seed_paths");
    let tail = &ORACLE_SRC[start + anchor.len()..];
    let end = tail.find("\npub fn ").unwrap_or(tail.len());
    let scope = &ORACLE_SRC[start..start + anchor.len() + end];

    assert!(
        scope.contains("landed"),
        "B270 M7: `validate_seed_paths` 今天只查路径不查字节——\
         铁律「已落位冻结 seed 不得修改」因此零机械后盾。\
         supersession 建在这块沙地上等于给一扇没锁的门配钥匙管理制度"
    );
}
