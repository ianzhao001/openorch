...
const LEGACY_SOURCE: &str = include_str!("../src/legacy.rs");
...
fn every_b306_public_receipt_and_reader_item_has_its_own_docs() {
    ...
    assert!(!TIERF_SOURCE.contains("pub struct NudgeOutcome"), "retired task-control type must not return");
