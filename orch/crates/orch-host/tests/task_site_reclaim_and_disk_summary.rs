...
fn task_site_fixture(tag: &str, task: &str, recorded_round: &str, current_round: &str) -> PathBuf {
    ...
    git(&root, &["init", "-q"]);
    git(&root, &["config", "user.email", "orch-test@example.invalid"]);
    ...
    for round in [recorded_round, current_round] {
        fs::create_dir_all(root.join(format!("coordination/rounds/{round}"))).expect("建轮目录失败");
    }
...
/// 按字符边界截断，语义等价于冻结契约里的 `&text[..len.min(limit)]`，
/// 但不会因为窗口边界落在多字节字符中间而 panic。
fn head_bytes(text: &str, limit: usize) -> &str {
...
fn dirty_worktree_is_reported_and_left_intact() {
    ...
    assert!(git_ok(&root, &["rev-parse", "--verify", "refs/heads/task/B902"]));
    assert!(
        root.join(".worktrees/B902-primary-executor-oc-g01").is_dir(),
...
fn sweep_targets_never_touches_the_main_debug_target() {
    ...
    let events = vec![leased_event("B905", "secondary", "executor-zc")];
    ...
    assert!(
        !target
            ...
            .exists(),
        "账本无 Active lease 引用的 review target 必须回收：{report:?}"
    );
    assert!(
        target
    ...
    assert!(
        !target.join("t-b204a3-codex").exists(),
        "白名单内的手工探针目录进回收面：{report:?}"
    );
    assert!(
...
fn sweep_targets_reports_freed_bytes() {
    ...
    let probe = "consult-01KZ409HF3FPC2B12YFV5VPEYK-0-consult-codex";
    let target = root.join("orch/target");
    ...
    let first = sweep_targets(&root, &[], Duration::from_secs(0)).expect("sweep-targets 不得失败");
    assert!(
    ...
    let second =
        sweep_targets(&root, &[], Duration::from_secs(0)).expect("重复 sweep 必须幂等成功");
    ...
    let big = sweep_targets(&bigger, &[], Duration::from_secs(0)).expect("sweep-targets 不得失败");
    assert!(
...
fn ungoverned_bytes_count_only_unmodeled_paths() {
    ...
    let entries = vec![
        DiskGovernanceEntry::new(".worktrees/B904", 18_500_000_000, Governance::ReclaimedThisRound),
    ...
    assert_eq!(summary.reclaimed_bytes, 20_500_000_000, "X 只计本轮回收掉的");
...
fn survey_disk_classifies_real_paths_so_the_gap_stays_visible() {
    ...
    write_blob(&root.join("orch/target/debug/deps"), "liborch_host.rlib", 8192);
    ...
    write_blob(&root.join("orch/target/test-tmp/b237-old"), "blob.bin", 2048);
    write_blob(
    ...
    let entries = survey_disk(&root, &reclaimed, &SweepTargetsReport::default())
        .expect("磁盘普查不得失败");
...
fn round_close_wires_reclaim_sweep_targets_and_summary() {
    // M4：读真实生产源码（`cleanup_wiring_and_logrotate.rs` 已冻结的裁定⑬范式）。
    // 本卡的能力如果没有生产调用者，就是第 N 次「交付了没人调」。
    // 后半段是**自护**：本卡的新增不得把老 needle 挤出同一个 8000 字节窗口。
    ...
    let head = head_bytes(close_region, 8000);
    ...
    for needle in [
        ...
        assert!(
            ...
            "收轮必须接线 {needle}（H111 的『谁调它』半句）"
        );
    }

    // 自护：老 needle 仍须落在窗口内，且留足余量——否则下一张卡再加两行就静默失效。
    let margin = head_bytes(close_region, 6000);
    for legacy in ["sweep_test_scratch_root", "reclaim_report"] {
        assert!(
            margin.contains(legacy),
            "本卡的新增把已冻结的老 needle {legacy} 挤出了窗口余量（阈值 6000 字节）"
        );
    }

    // 已冻结的 `cleanup_wiring_and_logrotate.rs` 用的是**裸字节切片**
    // `&close_region[..close_region.len().min(8000)]`——第 8000 字节落在多字节字符中间会直接
    // panic，而那个测试在 frozenPaths 里改不了。本卡要在窗口前部插入约 2KB，必须自己保住边界。
    assert!(
        close_region.len() <= 8000 || close_region.is_char_boundary(8000),
        "改动后第 8000 字节必须落在 char boundary 上，否则已冻结的 close_path_wires_* 会 panic"
    );
}
