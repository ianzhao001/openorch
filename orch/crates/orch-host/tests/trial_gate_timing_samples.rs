...
//! 多采样必须位于既有测试内部。B323 将 H56 演进为十样本的只读历史/纯决策门；
//! H75 仍是真实进程时序门，计数和判据不变；不通过重跑整个 collect 伪造多采样。
//!
...
fn assert_ten_sample_wrapper(source: &str, test_name: &str) {
    ...
    assert!(
        ...
        "{test_name} must execute each sample through its one-sample helper"
    );
}

#[test]
fn h75_retains_ten_samples_after_daemon_canary_retirement() {
    let manifest = Path::new(env!("CARGO_MANIFEST_DIR"));
    let wake = std::fs::read_to_string(manifest.join("src/wake.rs")).unwrap();
    assert_ten_sample_wrapper(&wake,
