...
fn card_byte_change_forces_revision_seed_reverify_and_new_signoff() {
    ...
    let first = support_legacy_plan::materialize(&site.root).unwrap();
    ...
    let second = support_legacy_plan::materialize(&site.root).unwrap();
...
fn expected_red_mismatch_and_record_only_leave_ledger_and_wal_unchanged() {
    ...
    support_legacy_plan::materialize(&site.root).unwrap();
...
fn invalid_card_crosschecks_leave_ir_and_validation_unchanged() {
    ...
    let target_error = support_legacy_plan::materialize(&site.root).unwrap_err().to_string();
    ...
    let frozen_error = support_legacy_plan::materialize(&site.root).unwrap_err().to_string();
...
fn b141_cycle_dependson_is_rejected_and_leaves_ir_and_revision_unchanged() {
    ...
    let error = support_legacy_plan::materialize(&site.root).unwrap_err().to_string();
