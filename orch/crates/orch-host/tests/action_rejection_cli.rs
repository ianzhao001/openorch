...
use orch_host::{inbox, ledger, serve, tierf, wake};
...
impl TestRepo {
    ...
    fn write_card(&self, task: &str, agent: &str) {
        let dir = self.root.join(format!("coordination/rounds/{ROUND}/tasks"));
        fs::create_dir_all(&dir).unwrap();
        fs::write(
            dir.join(format!("{task}.md")),
            format!(
                "---\ntaskId: {task}\nround: {ROUND}\nagent: {agent}\nwriteSet: []\nfrozenPaths: []\ngates: {{fast: []}}\n---\n# test\n"
            ),
        )
...
"#,
        ...
        .unwrap();
        orch_host::plan::run_plan(&self.root).unwrap();
...
#[test]
fn dispatch_bad_ledger_appends_a_durable_rejection_after_the_bad_line() {
...
#[test]
fn nudge_provider_spawn_fault_keeps_action_and_attempt_identity() {
    let site = TestRepo::new("nudge-spawn");
    site.activate_root_manual("B113", "executor-opencode");
    site.write_agents(serde_json::json!({
        "executor-opencode": {
            "injectable": true,
            "sessionId": "session-1",
            "wake": {
                "argv": ["/definitely/missing/b113-nudge-provider", "{session}", "{message}"]
            }
        }
    }));
    ledger::append(
        ...
        &[ledger::event(
            ...
            serde_json::json!({
                "agent": "executor-opencode",
                "baseSha": "0123456789abcdef",
                "goPath": "coordination/rounds/r47/dispatch/executor-opencode/GO-B113-A0001.md",
                "attemptId": "B113-A0001",
                "attemptNo": 1,
                "wakePending": true
            }),
    ...
    .err()
    .expect("nudge provider spawn must fail");
    let events = site.events();
    let payload = latest_rejection(&events);
    assert_eq!(payload["operation"], "nudge");
    assert_eq!(payload["exitCode"], 4);
    assert_eq!(payload["attemptId"], "B113-A0001");
    assert_eq!(payload["attemptNo"], 1);
    assert!(
        payload["actionId"]
            .as_str()
            .unwrap()
            .starts_with("nudge-r47-B113-B113-A0001-executor-opencode-"),
...
#[test]
fn run_task_adapter_spawn_fault_uses_current_attempt_identity() {
    let site = TestRepo::new("run-task-spawn");
    site.init_git();
    site.write_card("B113", "missing-provider");
    site.write_binding();
    let adapter_dir = site.root.join("coordination/adapters");
    fs::create_dir_all(&adapter_dir).unwrap();
    fs::write(
        adapter_dir.join("missing-provider.yaml"),
        "launch:\n  argv: [/definitely/missing/b113-tier-s-provider, \"{prompt}\"]\n  cwd_is_workdir: true\n",
    )
    .unwrap();

    let error = orch_host::run_task(&site.root, "B113", None, 1)
        .err()
        .expect("run-task provider spawn must fail");
    let events = site.events();
    let payload = latest_rejection(&events);
    assert_eq!(payload["operation"], "run-task");
    assert_eq!(payload["exitCode"], 2);
    assert_eq!(payload["actionId"], "B113-A0001");
    assert_eq!(payload["attemptId"], "B113-A0001");
    assert_eq!(payload["attemptNo"], 1);
    assert!(payload["reason"].as_str().unwrap().contains("spawn"));
    assert_error_matches_event(&error, payload);
}

#[test]
fn unsigned_daemon_once_is_rejected_before_planner_spawn() {
    let site = TestRepo::new("daemon-spawn");
    site.write_agents(serde_json::json!({
        "planner": {
            "injectable": true,
            "sessionId": "",
            "wake": {
                "argv": [],
                "freshArgv": [
                    "/definitely/missing/b113-planner-provider",
                    "--session-id",
                    "{session}",
                    "{message}"
                ]
    ...
    inbox::add(&site.root, "trigger planner", 1).unwrap();

    let error = serve::run_daemon(&site.root, true).unwrap_err();
    let events = site.events();
    let payload = latest_rejection(&events);
    assert_eq!(payload["operation"], "daemon");
    assert_eq!(payload["exitCode"], 5);
    assert_eq!(payload["actionId"], "daemon:r47:mechanical");
    let reason = payload["reason"].as_str().unwrap();
    assert!(
        reason.contains("ROUND-IR") || reason.contains("active") || reason.contains("modes"),
        "{reason}"
    );
...
#[test]
fn daemon_bad_ledger_and_reconcile_failure_are_both_durable() {
    let bad = TestRepo::new("daemon-bad-ledger");
    let ledger_path = bad
        .root
        .join(format!("coordination/rounds/{ROUND}/events.jsonl"));
    writeln!(
    ...
    .unwrap();
    let bad_error = serve::serve_tick(&bad.root).unwrap_err();
    let bad_read = orch_core::read_ledger(&ledger_path).unwrap();
    let bad_payload = latest_rejection(&bad_read.events);
    assert_eq!(bad_payload["operation"], "daemon");
    assert!(bad_payload["reason"].as_str().unwrap().contains("坏行"));
    assert_error_matches_event(&bad_error, bad_payload);

    let reconcile = TestRepo::new("daemon-reconcile");
    let lease_path = reconcile
        .root
        .join("coordination/runtime/locks/planner-turn.json");
    fs::create_dir_all(lease_path.parent().unwrap()).unwrap();
    fs::write(&lease_path, "not-json").unwrap();
    let reconcile_error = serve::reconcile_planner_turn(&reconcile.root).unwrap_err();
    let reconcile_events = reconcile.events();
    let reconcile_payload = latest_rejection(&reconcile_events);
    assert_eq!(reconcile_payload["operation"], "daemon-reconcile");
    assert_eq!(reconcile_payload["exitCode"], 5);
    assert!(reconcile_payload["reason"]
        .as_str()
        .unwrap()
        .contains("planner lease"));
    assert_error_matches_event(&reconcile_error, reconcile_payload);
}
