...
use orch_host::scheduler::{capacity_admits_in_domain, DomainAdmission};
use orch_host::util::test_scratch_dir;
use orch_host::wave::{plan_wave_schedule, split_wave_by_capacity_with_domains};
...
#[test]
fn wave_split_respects_shared_domain_not_per_agent_minimum() {
    // M5：波次规划同样要看域。两张卡分给同域两实例，各自 agent 上限 3、域上限 1
    //     ⇒ 不得同波派发（否则准入闸会在运行时把第二张卡挡回来，白跑一次分波）。
    let tasks = vec![
        ("B901".to_string(), vec!["a.rs".to_string()]),
        ("B902".to_string(), vec!["b.rs".to_string()]),
    ];
    let schedule = plan_wave_schedule(&tasks);
    assert_eq!(schedule.len(), 1, "writeSet 无冲突时二者本应同波（前提校验）");

    let agents = std::collections::HashMap::from([
        ("B901".to_string(), "executor-opencode".to_string()),
        ("B902".to_string(), "executor-opencode-scout".to_string()),
    ]);
    let capacities = std::collections::HashMap::from([
        ("executor-opencode".to_string(), 3usize),
        ("executor-opencode-scout".to_string(), 3usize),
    ]);
    let agent_domains = std::collections::HashMap::from([
        ("executor-opencode".to_string(), "opencode".to_string()),
        ("executor-opencode-scout".to_string(), "opencode".to_string()),
    ]);
    let domain_capacities = std::collections::HashMap::from([("opencode".to_string(), 1usize)]);

    let split = split_wave_by_capacity_with_domains(
        &schedule,
        &agents,
        &capacities,
        &agent_domains,
    ...
    .expect("按域分波不应失败");
    assert_eq!(split.len(), 2, "共享域上限 1 ⇒ 必须拆成两波，实得 {split:?}");
    assert_eq!(split[0].tasks.len(), 1);
    assert_eq!(split[1].tasks.len(), 1);
}
