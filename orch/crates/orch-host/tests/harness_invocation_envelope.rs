...
const WAKE_SRC: &str = include_str!("../src/channel/managed.rs");
...
fn all_three_wrappers_receive_the_same_envelope() {
    ...
    for (index, seen) in observed.iter().enumerate() {
        assert_eq!(
            seen, &first,
            "第 {index} 个 wrapper 收到的信封键集合与其它不一致"
        );
    }
...
fn the_envelope_is_validated_before_spawn() {
    let scope = window(WAKE_SRC, "fn spawn_managed_wake_supervisor_inner(", "\nfn ");
    let validate_at = scope
        .find("InvocationEnvelope::from_map")
        .unwrap_or_else(|| {
...
fn a_short_fixed_head_is_refused() {
    ...
    for bad in ["4d2ca5a8", "4D2CA5A800371C67158BDABC0E0FEC10A993EEE7"] {
        ...
        let error = InvocationEnvelope::from_map(envelope).expect_err("短 SHA / 大写 SHA 必须被拒");
...
fn executable_discovery_is_fail_closed() {
    ...
    assert!(
        !outcome
            .stderr
            .contains(&decoy.to_string_lossy().to_string()),
...
fn an_unbuilt_orch_binary_is_refused() {
    ...
    let error = InvocationEnvelope::from_map(envelope.clone()).expect_err("orchBin 不存在必须被拒");
