...
#[test]
fn a_structured_historical_event_survives_independent_production_replay() {
    for shape in [
        support::Shape::StructuredHappy,
        support::Shape::LiteralHappy,
    ] {
        let scene = support::emit_scene(shape);
        let payload=historical_payload_fixture(&scene.declaration, &scene.authorization,
            &scene.effective_main_sha, &scene.recorded.event_id);
        let emitted=vec![orch_host::ledger::event("FrozenContractSuperseded", "runtime:orch",
            Some(&scene.task_id), Some(&scene.round), serde_json::to_value(payload).unwrap())];
        ...
        assert_eq!(
            ...
            "{shape:?}: replay 重建的声明必须与发射时结构同构——\
             ...
             注意：这条走的是test-owned historical event→production replay，不是调用生产 writer 或自比较。"
        );
    }
}
...
// Test-owned historical data construction, never a production writer or authorization.
fn historical_payload_fixture(
    declaration: &orch_host::card::FrozenContractSupersession,
    authorization: &orch_host::verify::RootRecordAuthorization,
    effective_main_sha: &str,
    recorded_id: &str,
) -> orch_host::verify::FrozenContractSupersededPayload {
    let mut payload = serde_json::to_value(declaration).unwrap();
    payload.as_object_mut().unwrap().extend(
        serde_json::json!({
            "reviewBindings": authorization.reviews,
            "priorMainSha": authorization.expected_main_sha,
            "candidateSha": authorization.head_sha,
            "mergeSha": authorization.merge_sha,
            "effectiveMainSha": effective_main_sha,
            "irRevision": authorization.ir_revision,
            "validationDigest": authorization.validation_digest,
            "taskCardSha256": authorization.task_card_sha256,
            "planSignedOffEventId": authorization.plan_signed_off_event_id,
            "verdictEventId": authorization.verdict_event_id,
            "taskRecordedEventId": recorded_id,
        })
        .as_object()
        .unwrap()
        .clone(),
    );
    serde_json::from_value::<orch_host::verify::FrozenContractSupersededPayload>(payload).unwrap()
}
