...
fn all_production_rejection_callers_are_typed() {
    ...
    for path in paths {
        ...
        let file_count = typed_calls.iter()
            .map(|needle| compact.matches(needle).count()).sum::<usize>();
        typed_count += file_count;
        if file_count > 0 {
            by_file.insert(path.file_name().unwrap().to_str().unwrap().to_string(), file_count);
        }
    ...
    assert_eq!(
        typed_count, 11,
        "the eleven retained helper calls remain typed after retiring ten automatic/task-control callers"
    );
    assert_eq!(by_file, std::collections::BTreeMap::from([
        ("tierf.rs".to_string(), 5), ("wake.rs".to_string(), 6),
    ]), "retained caller closure must match exact source owners");
    assert_eq!(
