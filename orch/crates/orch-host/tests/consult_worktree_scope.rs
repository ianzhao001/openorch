...
/// 回归锁：consult 面板测试不得再对**主仓全局**计数下断言。
/// 这是 H59 的原始现场（`consult_fusion.rs` 在 `run_consultation` 前后各数一次
/// 主仓 `git worktree list` 并断言相等），与审查建场并发时必然假红。
#[test]
fn consult_fusion_no_longer_asserts_primary_repo_global_count() {
    ...
    assert!(
        source.contains("support_worktree"),
        "consult_fusion.rs 必须改用共享的作用域支撑件"
    );
}
