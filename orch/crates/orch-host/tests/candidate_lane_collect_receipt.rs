...
fn commit_synthetic_policy_activation(root: &Path, policy: &str) {
    let activated_at_main_sha = git(root, &["rev-parse", "main"]);
    let dormant = plan::resolve_runtime_policy_at(root, ROUND, policy, &activated_at_main_sha)
        .expect("fixture policy must resolve before activation");
    assert_eq!(dormant.state, plan::RuntimePolicyStateV1::Dormant);
    ...
    let recorded = events
        ...
        .find(|event| {
            event.kind == "TaskRecorded"
                && event.task_id.as_deref() == Some(dormant.owner_task.as_str())
        })
        .expect("policy owner must be recorded");
    let owner_merge_sha = events
        .iter()
        .rev()
        .find(|event| {
        ...
        .expect("policy owner must have merge SHA")
        .to_string();
    let payload = ledger::RuntimePolicyActivatedPayloadV1 {
        schema_version: ledger::RUNTIME_EVENT_SCHEMA_V1,
        policy: policy.to_string(),
        owner_task: dormant.owner_task,
        owner_recorded_event_id: recorded.event_id.clone(),
        owner_merge_sha,
        binding_sha256: dormant.binding_sha256,
        policy_sha256: dormant.policy_sha256,
        activated_at_main_sha,
    };
    let activation = ledger::event(
    ...
    serde_json::to_writer(&mut bytes, &activation).unwrap();
    bytes.push(b'\n');
    ...
    let activated_commit = commit_all(root, &format!("fixture activate {policy}"));
    let active = plan::resolve_runtime_policy_at(root, ROUND, policy, &activated_commit)
        .expect("committed synthetic activation must replay");
    assert_eq!(active.state, plan::RuntimePolicyStateV1::Active);
}
...
"#,
    ...
    if policy_active {
        commit_synthetic_policy_activation(&root, "candidate-lanes-v1");
    }
