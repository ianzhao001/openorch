...
//! M3. Return success/blocker exit instead of the dedicated awaiting-root exit 7.

use orch_core::EventRecord;
use orch_host::plan::{
    decode_task_validated, matching_plan_signoff, task_validated_payload,
};
use orch_host::wave::root_manual_wave_exit_code;
...
#[test]
fn awaiting_root_has_a_dedicated_non_success_exit() {
    assert_eq!(root_manual_wave_exit_code(false, 0), 0);
    assert_eq!(root_manual_wave_exit_code(true, 0), 1);
    assert_eq!(root_manual_wave_exit_code(false, 1), 7);
    assert_eq!(root_manual_wave_exit_code(true, 1), 1);
}
