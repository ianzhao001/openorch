...
//! ## The defect (H148)
//!
//! `scheduler::agent_inflight_load_from_events` keeps two projections:
...
use orch_host::scheduler::{agent_inflight_load_from_events, LoadKind};

// Compile-red until the live-request projection exists.
use orch_host::wake::current_review_request;

const SCHED: &str = include_str!("../src/scheduler.rs");
const WAKE: &str = include_str!("../src/wake.rs");
const SCHED_BYTES: &[u8] = SCHED.as_bytes();
...
fn frozen_source_contract_01() {
    assert!(
    count(SCHED_BYTES, b"reviews.remove(") >= 2,
...
fn frozen_source_contract_02() {
    assert!(
    count(SCHED_BYTES, b"\"AttemptBlocked\" | \"AttemptCrashed\"") == 1,
