...
fn a_successful_reclaim_exits_zero_with_the_invariant_holding() {
    let scene = support::scene_with_terminal_targets("b291-atomic", 2);
    let (code, out) = run_real_gc(&scene);
    let line = parse_gc_output(&out);
    assert!(
        line.removed > 0,
        "样本里有可回收对象，removed 必须大于 0；输出：{out}"
    );
    assert_eq!(
        line.refused, 0,
        "无 active lease 时不应有 REFUSED；输出：{out}"
    );
    assert!(
        line.holds,
        "物理回收成功后必须在同一收口内把 registry 记录剪掉并重算不变量；\
         `holds=false` 却报成功正是 H192 的第一条缺陷。输出：{out}"
    );
    assert_eq!(
        code, 0,
        "成功回收必须 exit 0（真实进程退出码）；输出：{out}"
    );
}
...
fn an_idempotent_replay_reports_zero_and_exits_zero() {
    let scene = support::scene_with_terminal_targets("b291-idempotent", 2);
    let (first_code, first_out) = run_real_gc(&scene);
    let first = parse_gc_output(&first_out);
    assert_eq!(first_code, 0, "首跑必须 exit 0；输出：{first_out}");
    assert!(
        first.removed > 0 && first.freed_bytes > 0,
        "首跑应有实际回收；输出：{first_out}"
    );

    let (second_code, second_out) = run_real_gc(&scene);
    let second = parse_gc_output(&second_out);
    assert_eq!(
        second.removed, 0,
        "已回收对象不得被重复计数；输出：{second_out}"
    );
    assert_eq!(
        second.freed_bytes, 0,
        "重放没有回收任何字节；输出：{second_out}"
    );
    assert_eq!(second_code, 0, "幂等重放必须 exit 0；输出：{second_out}");
    assert!(
        second.holds && second.prunable == 0,
        "已物理消失的对象不得继续算作 prunable；输出：{second_out}"
    );
}
...
fn freed_bytes_equals_the_measured_before_after_delta() {
    let known = 512 * 1024_u64;
    let scene = support::scene_with_sized_target("b291-bytes", known);
    let before = measured_bytes(&scene);
    let (code, out) = run_real_gc(&scene);
    let after = measured_bytes(&scene);
    let line = parse_gc_output(&out);
    assert_eq!(code, 0, "回收必须成功；输出：{out}");

    let measured = before.saturating_sub(after);
    assert!(
        line.freed_bytes >= known,
        "freedBytes 必须覆盖已知大小 {known}，实测 {}；输出：{out}",
        line.freed_bytes
    );
    assert!(
        line.freed_bytes <= before,
        "freedBytes 不得超过动作前的总占用 {before}（挡住 u64::MAX 之类的凭空值），实测 {}",
        line.freed_bytes
    );
    assert!(
        line.freed_bytes.abs_diff(measured) <= MEASUREMENT_TOLERANCE_BYTES,
        "freedBytes 必须等于动作前后按同一边界测量的饱和差（容差 {}）：\
         报告 {}，实测差 {measured}（before={before} after={after}）",
        MEASUREMENT_TOLERANCE_BYTES,
        line.freed_bytes
    );
}
...
fn an_active_lease_is_refused_and_exits_nonzero() {
    let scene = support::scene_with_active_lease("b291-active");
    let (code, out) = run_real_gc(&scene);
    let line = parse_gc_output(&out);
    assert!(line.refused > 0, "active lease 必须被 REFUSED；输出：{out}");
    assert_ne!(
        code, 0,
        "存在 REFUSED 时必须非零退出（真实进程）；输出：{out}"
    );
    assert!(
        scene
            .protected_target
            .as_ref()
            .is_some_and(|path| path.starts_with(&scene.root) && path.exists()),
        "active / dirty / blocked / debug 现场一个字节都不许动"
    );
}

#[test]
fn a_target_failure_still_exits_nonzero() {
    let scene = support::scene_with_undeletable_target("b291-failure");
    let (code, out) = run_real_gc(&scene);
    assert!(
        parse_gc_output(&out).target_failures > 0,
        "删除失败必须被计入 targetFailures；输出：{out}"
    );
    assert_ne!(
        code, 0,
        "存在 target failure 时必须非零退出（真实进程）；输出：{out}"
    );
}
