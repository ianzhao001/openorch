...
use orch_host::wake::{
    ...
    nudge_delivery_outcome, plan_managed_wake_attach, session_death_evidence_from_status,
    AttachMode, AttachRefusal, NudgeDelivery,
};
...
/// H64 的第三半：`orch nudge` **先写 NUDGE.md 再做 fence 校验**，
/// 所以「报错 ≠ 未送达」。这个真相以前只写在交接单里，现在必须由代码表达。
#[test]
fn nudge_reports_delivery_truthfully() {
    assert_eq!(
        nudge_delivery_outcome(/* written */ true, /* fence_refused */ true),
        NudgeDelivery::WrittenAwaitingConsumption,
        "已写盘但被 fence 拒的情形必须诚实自报「已送达待消费」，不得报成未送达"
    );
    assert_eq!(
        nudge_delivery_outcome(true, false),
        NudgeDelivery::Written,
        "正常写盘"
    );
    assert_eq!(
        nudge_delivery_outcome(false, true),
        NudgeDelivery::NotWritten,
        "确实没写盘才是未送达"
    );
}
