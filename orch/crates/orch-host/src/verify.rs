...
const NONGATE_RECEIPT_AGENTS: [&str; 2] = ["executor-antigravity", "executor-dsh"];
const NONGATE_RECEIPT_WARNING_PREFIX: &str = "[orch] nongate receipt warning:";
...
fn validate_nongate_attempt_receipt(
    root: &Path,
    round: &str,
    task_id: &str,
    attempt_id: &str,
    ...
    validate_nongate_attempt_receipt_binding(
        root, attempt_id, round, task_id, fixed_head, agent, events,
    )
    .map(|_| ())
}
...
fn nongate_receipt_warning_line(
    agent: &str,
    attempt_id: &str,
    fixed_head: &str,
    reason: &str,
    ...
    format!(
        "{NONGATE_RECEIPT_WARNING_PREFIX} agent={agent} attemptId={attempt_id} fixedHead={fixed_head} missing-or-invalid ({reason}); planner must explicitly adjudicate; verdict/seal remains non-blocking"
    )
}

/// Return one stable, non-blocking warning for every expected nongate seat
/// whose receipt is absent, malformed, or not bound to the exact durable
/// `WakeIssued` plus `WorkspaceLeased` pair. The expected seats come from the
/// standing nongate roster minus this signed attempt's formal reviewers; no
/// receipt problem is ever returned as an error to `verdict` or `seal`.
pub fn nongate_attempt_receipt_warnings(
    root: &Path,
    task_id: &str,
    attempt_id: &str,
    fixed_head: &str,
) -> Vec<String> {
    let all_warnings = |reason: &str| {
        NONGATE_RECEIPT_AGENTS
            .iter()
            .map(|agent| nongate_receipt_warning_line(agent, attempt_id, fixed_head, reason))
            .collect::<Vec<_>>()
    };
    let round = match crate::current_round(root) {
        Ok(round) => round,
        Err(error) => return all_warnings(&format!("current round unavailable: {error:#}")),
    };
    let ledger_path = root.join(format!("coordination/rounds/{round}/events.jsonl"));
    let ledger = match read_ledger(&ledger_path) {
        Ok(ledger) if ledger.bad_lines.is_empty() => ledger,
        Ok(_) => return all_warnings("round ledger contains bad lines"),
        Err(error) => return all_warnings(&format!("round ledger unavailable: {error:#}")),
    };
    let active = match crate::plan::require_active_round_ir(root, &round, &ledger.events) {
        Ok(active) => active,
        Err(error) => return all_warnings(&format!("signed ROUND-IR unavailable: {error:#}")),
    };
    let task = match active
        .candidate
        .tasks
        .iter()
    ...
    {
        Some(task) => task,
        None => return all_warnings("task is absent from signed ROUND-IR"),
    };
    let formal_reviewers = task
        .required_reviews
        .iter()
        .map(|review| review.agent.as_str())
        .collect::<BTreeSet<_>>();

    let expected_agents = if task.review_quorum.is_some() {
        task.nongate_seats
            .iter()
        ...
        NONGATE_RECEIPT_AGENTS
            .iter()
            .copied()
            .filter(|agent| !formal_reviewers.contains(*agent))
            .collect::<Vec<_>>()
    };
    ...
    expected_agents
        .into_iter()
        .filter_map(|agent| {
            validate_nongate_attempt_receipt(
                root,
                &round,
                task_id,
                attempt_id,
            ...
            .err()
            .map(|reason| nongate_receipt_warning_line(agent, attempt_id, fixed_head, &reason))
        })
        .collect()
}
...
pub fn review_contract_mode_for_attempt(
    ...
    let resolved = match crate::plan::resolve_attempt_runtime_policy(
        ...
        Err(error)
            if error.to_string().contains("缺 runtime policy review-pool-v1")
                || error
                    .to_string()
                    .contains("缺 runtimePolicies envelope")
                || {
...
pub fn evaluate_review_quorum(
    ...
    let Some(first) = results.first() else {
        return ReviewQuorumDecision::Insufficient;
    };
    ...
    for result in results {
        ...
        match result.state {
            ...
            | ReviewResultState::TimedOut
                if nongate => {}
            _ => return ReviewQuorumDecision::Invalid,
        }
    }
    if finding {
        return ReviewQuorumDecision::BlockedByFinding;
    }
    if !evaluate_seat_closure(
        policy,
        pending_formal,
    ...
    if substantive >= policy.minimum_substantive {
...
/// Exact durable payload for the sole runtime-authorized mutation of an
/// already-landed frozen contract.  The signed declaration is repeated
...
impl FrozenContractSupersededPayload {
    /// Construct the durable event payload from one exact signed declaration
    /// and its already-validated root record authorization.
    ///
    /// Both authorization arms are copied without normalization: historical
    /// recovery anchors keep their old top-level JSON keys, while a planner
    /// adjudication remains the sole authorization and never receives forged
    /// recovery identities.
    pub fn from_signed_declaration(
        declaration: &card::FrozenContractSupersession,
        authorization: &RootRecordAuthorization,
        effective_main_sha: &str,
        task_recorded_event_id: &str,
    ) -> Self {
        Self {
            target: declaration.target.clone(),
            initiator: declaration.initiator.clone(),
            original_seed_relocated: declaration.original_seed_relocated.clone(),
            effective_anchor: declaration.effective_anchor.clone(),
            old_file_sha256: declaration.old_file_sha256.clone(),
            new_file_sha256: declaration.new_file_sha256.clone(),
            old_literal_sha256: declaration.old_literal_sha256.clone(),
            new_literal_sha256: declaration.new_literal_sha256.clone(),
            subject_prefix: declaration.subject_prefix.clone(),
            structured_evolution: declaration.structured_evolution.clone(),
            blocked_attempt: declaration.blocked_attempt.clone(),
            replacement: declaration.replacement.clone(),
            authorization: declaration.authorization.clone(),
            dispositions: declaration.dispositions.clone(),
            reviews: declaration.reviews.clone(),
            review_bindings: authorization.reviews.clone(),
            prior_main_sha: authorization.expected_main_sha.clone(),
            candidate_sha: authorization.head_sha.clone(),
            merge_sha: authorization.merge_sha.clone(),
            effective_main_sha: effective_main_sha.to_string(),
            ir_revision: authorization.ir_revision,
            validation_digest: authorization.validation_digest.clone(),
            task_card_sha256: authorization.task_card_sha256.clone(),
            plan_signed_off_event_id: authorization.plan_signed_off_event_id.clone(),
            verdict_event_id: authorization.verdict_event_id.clone(),
            task_recorded_event_id: task_recorded_event_id.to_string(),
        }
    }
...
impl ReviewContractExpectation {
    ...
    pub fn panel_exact(
        ...
        let mut expectation = Self::exact(
            task_id,
            round,
            attempt_id,
            role,
...
fn validate_archived_review_transition_chain(
    ...
    for required in &task.required_reviews {
        ...
        let terminal =
            managed_channel_terminal(prefix, round, task_id, &required.agent, source_wake_id)?
        ...
        if fallback_binding.is_none() && substituted_fallback.is_none() {
            bail!("archived fallback selection 未被 root binding 消费或耗尽替席");
        }
    }

    for binding in root_payload
        .reviews
        .iter()
        .filter(|binding| binding.role == "nongate")
    {
        ...
        let deliveries = prefix
            .iter()
            .filter(|event| {
                event.event_id == delivery_event_id && event.kind == "NongateReviewDelivered"
            })
            .collect::<Vec<_>>();
        let [delivery] = deliveries.as_slice() else {
            bail!("archived nongate deliveryEventId 不唯一");
        };
        if delivery.actor != "runtime:orch"
            ...
            || nongate_event_payload_str(delivery, "agent") != Some(binding.reviewer.as_str())
        ...
        let seat = task
            .nongate_seats
            .iter()
            .find(|seat| seat.agent == binding.reviewer)
            .expect("signed seat checked above");
        let receipt = validate_nongate_attempt_receipt_binding(
            root,
            &root_payload.attempt_id,
            round,
            task_id,
        ...
        {
            bail!("archived nongate receipt/preset 与 delivery/signed seat 漂移");
        }
    }
    Ok(())
}
...
pub fn audit_recorded_review_bindings(root: &Path) -> Result<ReviewBindingAuditReport> {
    ...
    for ledger_rel in ledgers {
        ...
        for (task_id, recorded_position) in recorded {
            ...
            let historical_ir: crate::plan::RoundIr = serde_yaml::from_slice(&ir_bytes)
                .context("doctor historical ROUND-IR 非 canonical")?;
            ...
            for binding in payload.reviews.clone() {
                ...
                let expected = if review_mode == ReviewContractMode::Panel {
                    let promotions = events
                        .iter()
                        .filter_map(|event| {
                            let Ok(Some(crate::ledger::RuntimeEventPayloadV1::ReviewSpoolPromoted(
                                promotion,
...
fn recorded_frozen_authorization(
    ...
    let historical_ir: crate::plan::RoundIr = serde_yaml::from_slice(&ir_bytes)
        .context("frozen supersession historical ROUND-IR 非 canonical")?;
...
pub(crate) fn frozen_contract_card_from_record_authorization(
    ...
    let historical_ir: crate::plan::RoundIr = serde_yaml::from_slice(&ir_bytes)
        .context("frozen authorization historical ROUND-IR 非 canonical")?;
...
pub(crate) fn pending_frozen_record_context_from_committed_main(
    ...
    let historical_ir: crate::plan::RoundIr = serde_yaml::from_slice(&ir_bytes)
        .context("pending frozen historical ROUND-IR 非 canonical")?;
    if historical_ir.round != round
...
/// Build the Effective events for one already-authorized, post-green record
/// edge.  The returned order is stable by target; no empty-card compatibility
/// path can synthesize a lifecycle event.
pub fn build_frozen_contract_superseded_events(
    root: &Path,
    round: &str,
    task_id: &str,
    ...
    if authorization.already_recorded {
        bail!("已 Recorded task 不得重复发射 FrozenContractSuperseded");
    }
    let mut declarations =
        frozen_contract_card_from_record_authorization(root, round, task_id, authorization)?
            ...
            .frozen_contract_supersessions;
    declarations.sort_by(|left, right| left.target.cmp(&right.target));
    // Historical empty cards emit no Frozen fact, so their ordinary record
    // recovery keeps the pre-B270 rule: gates may run at the captured merge
    // even after an unrelated main advance.  A non-empty declaration changes
    // the landed effective state and must still bind the captured current tip.
    if declarations.is_empty() {
        return Ok(Vec::new());
    }
    full_sha(effective_main_sha, "effectiveMainSha")?;
    if crate::gitx::rev_parse(root, "main")? != effective_main_sha {
        bail!("FrozenContractSuperseded effectiveMainSha 必须是捕获的 current main");
    }
    let mut events = Vec::with_capacity(declarations.len());
    for declaration in declarations {
        if !declared_reviews_match_bindings(&declaration.reviews, &authorization.reviews) {
            bail!("FrozenContractSuperseded declaration reviews 未绑定 root PASS review digests");
        }
        crate::oracle::validate_frozen_contract_declaration(
            root,
            &declaration,
    ...
    let suffix = std::iter::once(recorded.clone())
        .chain(events.iter().cloned())
        .collect::<Vec<_>>();
    validate_frozen_contract_supersession_suffix(root, round, existing, &suffix)?;
    Ok(events)
}

/// Ledger admission hook: an exclusive capability is necessary but not
/// sufficient.  Any proposed Frozen event must belong to the exact complete
/// TaskRecorded group reconstructed from signed historical facts.
...
fn canonical_wake_backend_receipt(
    ...
    if !matches!(provider, "codex" | "opencode" | "smartclaw" | "pi" | "zcode" | "agy")
    ...
    let signed_pin = matches!(provider, "pi" | "zcode" | "agy");
...
fn current_formal_request<'a>(
    ...
    let wakes = events
        ...
        .collect::<Vec<_>>();
    let [(wake_position, _)] = wakes.as_slice() else {
        bail!("formal ReviewRequested 缺唯一、先行的 exact WakeIssued");
    };
    if *wake_position >= request_position {
...
fn managed_channel_terminal<'a>(
    ...
    if matches!(outcome, "OperationalError" | "StoppedByAuthenticatedCancel") {
        return Ok(None);
    }
    Ok(Some(terminal))
}
...
fn quorum_file_bindings(
    ...
    for required in &ir_task.required_reviews {
        ...
        if let Some((selection_position, selection)) = selections.first().copied() {
            ...
            if managed_channel_terminal(events, round, task_id, &required.agent, source_wake_id)?
                .is_none_or(|event| event.event_id != terminal.event_id)
            {
        ...
        let wake_id = nongate_event_payload_str(request, "wakeId")
            .context("formal ReviewRequested 缺 wakeId")?;
        let terminal = managed_channel_terminal(events, round, task_id, reviewer, wake_id)?;
        let state = if terminal.is_some() && (fallback_selected || signed_fallback.is_none()) {
            ReviewResultState::ChannelError
        } else {
    ...
    let decision = evaluate_review_quorum(policy, &results);
    ...
    let mut substitutions = Vec::new();
    if verdict == RootVerdict::Pass {
        if formal_errors.len() > nongate_pass_bindings.len() {
...
fn panel_file_bindings(
    ...
    if verdict == RootVerdict::Pass {
        for evidence_id in &ir_task.required_evidence {
            let rel = format!("coordination/rounds/{round}/evidence/{task_id}-{evidence_id}.json");
            let bytes = regular_file_bytes(root, &root.join(&rel), "required evidence")?;
            let committed = committed_regular_blob_bytes(root, main_sha, &rel, "required evidence")?;
            if bytes != committed {
                bail!("required evidence 当前 bytes 与 expected main blob 不一致: {rel}");
            }
            let _: serde_json::Value = serde_json::from_slice(&bytes)
...
fn required_file_bindings(
    ...
    let mode = review_contract_mode_for_attempt(
        root,
        round,
        events,
        task_id,
...
fn attempt_runtime_policy_active(
    ...
    match crate::plan::resolve_attempt_runtime_policy(
        ...
        Err(error) => Err(error)
            .with_context(|| format!("resolve attempt runtime policy {policy} failed")),
    }
}
...
fn capture_root_gate_subject_v1(
    ...
    Ok(CapturedRootGateSubjectV1 {
        subject: RootGateSubjectV1 {
            subject_tree_sha: crate::gitx::rev_parse(
                root,
                &format!("{expected_head}^{{tree}}"),
            )?,
...
fn plan_live_root_gate_reuse_v1(
    ...
    let validated = match crate::tierf::load_validated_collect_gate_bundle_v1(
        root, round, task_id, attempt_id,
    ) {
        ...
        Err(error) => {
            ...
            return root_gate_reuse_miss(
                ...
                detail.clone(),
                root_gate_reuse_json_sha256(
                    "root-gate-reuse-unreadable-receipt-v1",
                    &detail,
                )?,
            );
        }
    ...
    Ok(LiveRootGateReusePlanV1::Reuse(
        PreparedRootGateReuseV1 {
            bundle,
            subject: captured.subject,
            gates,
...
fn read_root_reuse_collect_attestation(
    ...
    let attestation: RootReuseCollectAttestationV1 =
        serde_json::from_slice(&bytes).context("root reuse receipt attestation 非 canonical JSON")?;
...
fn validate_reused_gate_event_and_source(
    ...
    let attestation = read_root_reuse_collect_attestation(root, receipt)?;
...
fn validate_existing_verdict_gates(
    ...
    let reuse_active =
        root_gate_reuse_policy_active(root, round, events, task_id, attempt_id)?;
    ...
    if reuse_count > 0 {
        ...
        let prepared = match plan_live_root_gate_reuse_v1(
            ...
            LiveRootGateReusePlanV1::Execute(miss) => {
                bail!("existing GateReused verdict no longer validates: {}", miss.detail)
            }
        };
...
fn run_root_verdict_locked(
    ...
    let reuse_active = root_gate_reuse_policy_active(
        root,
        &round,
        &lr.events,
        task_id,
...
fn validate_archived_root_gate_bindings_v1(
    ...
    let receipt_matches = events[..*first_position]
        ...
        .collect::<Vec<_>>();
    let [receipt] = receipt_matches.as_slice() else {
    ...
    let card_bytes = regular_file_bytes(root, &root.join(&card_rel), "archived root reuse card")?;
    let (current_card_sha256, _) = sha_binding(&card_bytes);
    if current_card_sha256 != *historical_card_sha256 {
        bail!("archived root reuse current card bytes 漂移");
    }
    let bound_card = card::load(root, round, task_id)?;
    let dispatch = crate::attempt::resolve_current_dispatch(events, task_id, round)?;
    let policy_base_sha = dispatch
        .base_sha
...
pub fn validate_archived_record_chain(
    ...
    for (index, gate) in root_payload.gates.iter().enumerate() {
        if gate.name.is_empty()
            || (gate.reused_event_id.is_some() && gate.log_bytes == 0)
        {
...
"#,
    ...
    fn refused_verdict_retry_recovers_and_reaches_the_real_gate_spawn() {
        ...
        let log_prefix = "B172T-round-r58-B172T-A0001-root-";
        ...
        .unwrap();
        for path in matching_gate_logs(&log_dir, log_prefix, log_suffix) {
        ...
        assert!(precheck.gates.is_empty());
        assert!(
            matching_gate_logs(&log_dir, log_prefix, log_suffix).is_empty(),
            "dry-run must return before storage admission and gate spawn"
        );
        ...
        assert!(
            matching_gate_logs(&log_dir, log_prefix, log_suffix).is_empty(),
            "refusal must happen before every phase-scoped gate log/spawn"
        );
        let events = site.events();
        ...
        let gate = retry
            ...
            .expect("retry must return the real gate result");
        let logs = matching_gate_logs(&log_dir, log_prefix, log_suffix);
        let [log_path] = logs.as_slice() else {
            panic!("retry must create exactly one phase-scoped gate log: {logs:?}");
        };
        ...
        let gate_run_id = file_name
            ...
            .expect("root retry log must retain the typed prefix/suffix");
