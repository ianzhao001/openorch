...
pub fn run_gate_with_audit_identity(
    ...
    // The storage audit is owned by `root`, but the execution capability must
    // bind the repository that actually hosts the child. Linked worktrees
    // fold to the same common-dir identity; an oracle isolated clone does not.
