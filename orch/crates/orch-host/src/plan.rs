...
const CURRENT_ROUND_IR_SCHEMA_VERSION: u32 = 2;
...
fn is_legacy_round_ir_schema_version(version: &u32) -> bool {
    *version == LEGACY_ROUND_IR_SCHEMA_VERSION
}
...
fn compile_ir_with_dispatched(
    ...
    Ok(RoundIr {
        schema_version: CURRENT_ROUND_IR_SCHEMA_VERSION,
...
pub fn parse_signed_round_ir(yaml: &str) -> std::result::Result<RoundIr, String> {
    let ir: RoundIr = serde_yaml::from_str(yaml)
        .map_err(|error| format!("解析 signed ROUND-IR 失败: {error}"))?;
    if !matches!(
        ir.schema_version,
        LEGACY_ROUND_IR_SCHEMA_VERSION | CURRENT_ROUND_IR_SCHEMA_VERSION
    ) {
        return Err(format!(
            "ROUND-IR schemaVersion={} 未建模（仅支持 legacy 1 与 current 2）",
            ir.schema_version
        ));
    }
    Ok(ir)
...
fn load_task_inputs(root: &Path, round: &str, purpose: TaskLoadPurpose) -> Result<LoadedTasks> {
    ...
    for task_id in task_ids {
        ...
        let entry_points_required = round_number_at_least(round, 53);
        ...
        if round_number_at_least(round, 57) {
...
pub fn require_active_round_ir(
    ...
    if persisted.policy.push_policy.is_empty()
...
fn run_plan_locked(root: &Path) -> Result<PlanOutcome> {
    ...
    let digest = validation_digest(&ir);
    if ir_written {
        let yaml = serde_yaml::to_string(&ir).context("序列化 ROUND-IR 失败")?;
...
"#;
    ...
    fn run_plan_accepts_a_head_declared_new_module() {
        ...
        let outcome = run_plan(&root).expect("HEAD declaration makes the new module reachable");
    ...
    fn run_plan_accepts_a_new_module_when_the_card_owns_lib_rs() {
        ...
        let outcome = run_plan(&root).expect("a card that owns lib.rs can add the declaration");
        assert!(outcome.ir_written || outcome.event_appended);
        let _ = fs::remove_dir_all(root);
    }
    ...
    fn run_plan_never_flags_cargo_discovered_files() {
        ...
        let outcome =
            run_plan(&root).expect("tests/*.rs, build.rs, and src/main.rs need no lib declaration");
        assert!(outcome.ir_written || outcome.event_appended);
        let _ = fs::remove_dir_all(root);
    }
    ...
    fn same_ir_file_conflict_accepts_declared_dependency_pair_at_plan() {
        ...
        let outcome = run_plan_locked(&root).unwrap();
        assert!(outcome.ir_written || outcome.event_appended);
        assert!(
            root.join("coordination/rounds/r49/ROUND-IR.yaml").exists(),
