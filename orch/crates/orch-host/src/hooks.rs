...
mod tests {
    ...
    fn r82_ledger() -> String {
        fs::read_to_string(source_repo_root().join("coordination/rounds/r82/events.jsonl"))
            .expect("读取真实 r82 ledger")
    }

    fn commit_round_fixture_before_guard(root: &Path, round: &str, ledger: &str) -> String {
        write_active_round(root, round, ledger);
        fs::write(root.join("tracked.txt"), "base\n").unwrap();
        git(root, &["add", "-A"]);
        git(root, &["commit", "--quiet", "-m", "fixture before guard"]);
        git(root, &["rev-parse", "HEAD"])
    }

    #[test]
    fn r82_taskless_one_shot_allows_followup_main_commit() {
        let ledger = r82_ledger();
        assert_eq!(ledger.lines().count(), 790);
        assert!(ledger.contains("01M19R8TGFR3GQKJAE46AADDA1"));
        assert!(ledger.contains("01a05384-6682-4616-9d77-9cebcf6a652d"));
        assert!(ledger.contains("01M1BQQV3NGGXCENAKWNJM2HPV"));

        let root = temp_root("r82-taskless-one-shot");
        init_repo(&root);
        let before = commit_round_fixture_before_guard(&root, "r82", &ledger);
        ensure_main_guard(&root).unwrap();

        let after = commit_file(&root, "after\n", "taskless wake does not own round close");
        assert_ne!(after, before);
        assert!(violation_records(&root).is_empty());
    }

    #[test]
    fn r82_missing_task_wake_terminal_still_blocks_even_when_forced() {
        let ledger = r82_ledger();
        let terminal_event = "01M1BNZ9RP5G5WCYCQ7XJ8RCVR";
        let task_wake = "01a0568f-fcbc-4149-ab09-d3f96da50504";
        let mut removed = 0;
        let mutated = ledger
            .lines()
            .filter_map(|line| {
                if line.contains(terminal_event) {
                    removed += 1;
                    None
                } else {
                    Some(format!("{line}\n"))
            ...
            .collect::<String>();
        assert_eq!(removed, 1);
        assert!(mutated.contains(task_wake));
        assert!(mutated.contains("\"forced\":true"));

        let root = temp_root("r82-missing-task-terminal");
        init_repo(&root);
        let before = commit_round_fixture_before_guard(&root, "r82", &mutated);
        ensure_main_guard(&root).unwrap();

        let blocked = blocked_commit(
            &root,
            "blocked\n",
            "forced close must keep task wake strict",
        );
        let stderr = String::from_utf8_lossy(&blocked.stderr);
        assert!(
            stderr.contains("RoundClosed phase/envelope is not canonical"),
            "{stderr}"
        );
    ...
    #[test]
    fn taskless_wake_still_blocks_legacy_runtime_policy_transition() {
        let ledger = r82_ledger();
        let taskless_event = "01M19R8TGFR3GQKJAE46AADDA1";
        let policy = event_line(
            "01TESTTASKLESSPOLICY0000000",
            "runtime:orch",
            "RuntimePolicyActivated",
            None,
            "r82",
            serde_json::json!({
                "activatedAtMainSha": "cb4d06e7a290a1e3a6264281561306a36ccd8129",
                "bindingSha256": "eeaa95ef2ef4f7e7b86e2cdcdd1222f6ec9d78ad8de0ff80b7eafbf464c9cb39",
                "ownerMergeSha": "bd9477c3867150983ce71d0f34605f75fc581519",
                "ownerRecordedEventId": "01M156PR1PBS0HAZPQBYD3M6ZG",
                "ownerTask": "B311",
                "policy": "review-pool-v1",
                "policySha256": "6c0e7d727e0d72ac945fcc37b1c81761001714b810c1e64bbfdd889bee6306e6",
                "schemaVersion": 1,
            }),
        );
        ...
        for line in ledger.lines() {
            ...
            injected.push('\n');
            if line.contains(taskless_event) {
                injected.push_str(&policy);
                insertions += 1;
            }
        }
        assert_eq!(insertions, 1);

        let root = temp_root("r82-taskless-policy-idle");
        init_repo(&root);
        let before = commit_round_fixture_before_guard(&root, "r82", &injected);
        ensure_main_guard(&root).unwrap();

        let blocked = blocked_commit(&root, "blocked\n", "taskless wake keeps global idle strict");
        let stderr = String::from_utf8_lossy(&blocked.stderr);
        assert!(
            stderr.contains(
                "RuntimePolicyActivated is forbidden while attempt/wake/barrier is in flight"
            ),
            "{stderr}"
        );
        assert_eq!(git(&root, &["rev-parse", "HEAD"]), before);
