...
impl TestScratchGateLane {
    fn acquire(root: &Path, entry: GuardEntry) -> Option<Self> {
        if entry != GuardEntry::Gate || !is_direct_test_scratch_repository(root) {
            return None;
        }
...
impl StorageSettings {
    ...
    fn estimate(self, entry: GuardEntry) -> u64 {
        match entry {
            ...
            GuardEntry::Gate => self.gate_estimate_bytes,
            GuardEntry::Reclaim | GuardEntry::RoundClose => 0,
        }
    }
}
...
mod tests {
    ...
    fn verifier_storage_suffix_accepts_only_exact_gate_audit_shapes() {
        ...
        assert!(ledger::canonical_gate_storage_audit_event(&refused).is_some_and(|audit| {
            audit.round == "r-test" && audit.task_id == "B-test"
        }));
        ...
        assert!(ledger::canonical_gate_storage_audit_event(&failed).is_some_and(|audit| {
            audit.round == "r-test" && audit.task_id == "B-test"
        }));
        ...
        assert!(ledger::canonical_gate_storage_audit_event(&recovered).is_some_and(|audit| {
            audit.round == "r-test" && audit.task_id == "B-test"
        }));
        ...
        for (label, mut forged) in [
            ...
            match label {
                ...
                "low value relation" => {
                    forged.payload.as_mut().unwrap()["availableBytes"] =
                        serde_json::json!(50 * GIB)
                }
                "extra payload key" => {
                    forged.payload.as_mut().unwrap()["forged"] = serde_json::json!(true)
                }
                "extra top-level key" => {
                    forged.extra.insert("forged".into(), serde_json::json!(true));
                }
                ...
                "recovered low available" => {
                    forged.payload.as_mut().unwrap()["availableBytes"] =
                        serde_json::json!(45 * GIB)
                }
                "recovered non-null reason" => {
                    forged.payload.as_mut().unwrap()["probeReason"] =
    ...
    fn position_after(source: &str, start: &str, needle: &str) -> usize {
        ...
        start + source[start..].find(needle).expect("entry operation missing")
    }
    ...
    fn production_guards_precede_each_entrys_first_effect_boundary() {
        ...
        assert!(
            position_after(&tierf, dispatch, "crate::storage::guard_operation")
                < position_after(&tierf, dispatch, "crate::scheduler::with_capacity_lock")
        );
