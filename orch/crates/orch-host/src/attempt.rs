...
/// 最近终态 attempt 的实现者；历史 `ReviewPanelClosed(pool-exhausted)` 的
/// 相邻 runtime-owned AttemptBlocked 可使用 panel agent，其余终态不得覆盖实现者。
fn latest_terminal_attempt_agent(events: &[EventRecord], task: &str) -> Result<Option<String>> {
    ...
    let panel_runtime_terminal = match terminal_position {
        Some(position) => crate::legacy::canonical_review_panel_exhaustion_terminal_for_successor(
...
/// Approved-reattempt compatibility wrapper: reject historical generations before leasing and recheck the captured round inside the lease.
/// Tier-F owns nested capacity admission and atomic `AttemptBlocked + DispatchIssued`; Existing finishes dispatch-wake accounting before exact successor validation.
/// Current callers use explicit v3 local/harness routes.
pub fn run_approved_reattempt_dispatch(
    ...
    let guarded_round = crate::tierf::require_current_open_schema3_dispatch_generation(root, None)?;
    crate::close::with_protocol_transition_named(
        root,
        "orch dispatch --new-attempt",
        Some(task_id),
        || {
            crate::tierf::require_current_open_schema3_dispatch_generation(root, Some(&guarded_round))?;
