...
pub struct InvocationEnvelope {
    ...
    /// Authorized invocation role (`primary`, `secondary`, `nongate`, or `implement`).
    pub role: String,
...
impl InvocationEnvelope {
    ...
    pub fn from_map(mut map: BTreeMap<String, String>) -> Result<Self> {
        ...
        if !matches!(
            role.as_str(),
            "primary" | "secondary" | "nongate" | "implement"
        ) {
        ...
        let review_output = if review_output_text == NO_REVIEW_OUTPUT {
            if role != "implement" {
                bail!("ORCH_HARNESS_REVIEW_OUTPUT_PATH sentinel is valid only for implement role");
            }
            None
        } else {
            let path = PathBuf::from(&review_output_text);
            if role == "implement" || !path.is_absolute() {
                bail!(
