...
pub enum FailureClass {
    ...
    Timeout,
    Protocol,
    /// A trusted driver error reports that the upstream response stream failed.
    UpstreamStream,
    /// Runtime observation failed; this does not establish process termination.
    ObservationFailed,
    Unknown,
}

impl FailureClass {
    /// Stable diagnostic wire name; existing names retain their historical spelling.
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::UpstreamStream => "upstream-stream",
