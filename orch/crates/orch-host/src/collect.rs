...
use crate::{binding, buildcache, card, gate, gitx, ledger, mech, oracle, plan, srcshape};
...
fn build_closed_candidate_commands(
    ...
    closure_targets: &[srcshape::SourceReaderTargetV1],
) -> Result<Vec<ResolvedCollectCommand>> {
...
pub(crate) fn resolve_collect_lane_plan(
    ...
    if let Some(prior) = prior_escalations.first() {
        ...
        let source_reader_digests =
            srcshape::authenticated_source_reader_digests_v1(root, policy_base_sha).ok();
        return Ok(CollectLanePlan {
            commands,
            trial_refs,
            committed_binding: committed,
    ...
    let source_changed = changed_paths
        ...
        .cloned()
        .collect::<Vec<_>>();
    let closure = srcshape::resolve_source_reader_closure_at_tree_v1(
        root,
        round,
        candidate_sha,
        policy_base_sha,
    ...
    let (closure_targets, closure_reason, descriptor_sha256, base_sha256) = match closure {
        srcshape::SourceReaderClosureDecisionV1::Closed {
            targets,
            descriptor_sha256,
            base_sha256,
        } => (targets, None, Some(descriptor_sha256), Some(base_sha256)),
        srcshape::SourceReaderClosureDecisionV1::UpgradeToFast {
            reason,
            descriptor_sha256,
            base_sha256,
        } => (
