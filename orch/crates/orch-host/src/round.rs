...
/// `orch round open <id>`：目录骨架(O6) → RoundOpened(+可选 PlanSignedOff) → CURRENT-ROUND → BOARD 开版行
pub fn run_open(
    root: &Path,
    id: &str,
    purpose: &str,
    signed_off: bool,
    sign_note: Option<&str>,
    force: bool,
) -> Result<OpenOutcome> {
    crate::close::with_protocol_transition(root, "orch round open", || {
        run_open_locked(root, id, purpose, signed_off, sign_note, force)
    })
}
...
fn run_open_locked(
    ...
    if signed_off {
        bail!("round open --signed-off 不再产生未绑定签核；先 orch plan，再 orch round sign-off");
    }
    let _ = sign_note;
    ...
    let already_opened = new_ledger.is_file()
        && read_ledger(&new_ledger)?
            .events
            .iter()
            .any(|e| e.kind == "RoundOpened");
    ...
    let mut payload = serde_json::json!({ "purpose": purpose });
    if force {
        if let Some(prev) = &pointer {
