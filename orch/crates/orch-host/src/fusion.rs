//! Legacy AdapterSpec fusion helpers retained for historical tests/readers.
//!
//! Schema-3 `consult` does not call this fanout path: explicit aliases run
//! through the unified channel in `consult.rs`. Answer extraction remains a
//! shared, provider-neutral helper.

/// Shared answer parsing retains the historical fusion API path.
pub use crate::consult::{extract_member_answer, ANSWER_BODY_LIMIT};
