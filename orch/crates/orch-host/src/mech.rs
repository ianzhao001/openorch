...
const LOCAL_IDENTITY_FINGERPRINT: &str =
    "a21aa49b9b34b503633e0f68debfb49edac0e020ff8d3e91a0c288edb508afd9";
...
fn local_identity_audit(decl: &EnvDecl) -> ReportIdentityAudit {
    let basis = IdentityAuditBasis {
        source: "local".to_string(),
        tool: None,
        registry_fingerprint: LOCAL_IDENTITY_FINGERPRINT.to_string(),
        wake_event_id: None,
        receipt_event_id: None,
    };
    ReportIdentityAudit {
        payload: serde_json::json!({
            "policy": "actorless-local",
            "declared": {"model": null, "effort": null, "origin": "local"},
            "selfReported": {
                "model": decl.model,
                "effort": decl.depth,
                "capture": decl.capture,
            ...
            "observed": {"status": "not-applicable", "source": "local"},
            "outcome": {"status": "local"},
            "verificationStatus": "local",
            "evidenceReady": true,
            "evidenceBasis": {
                "source": "local",
                "tool": null,
                "registryFingerprint": LOCAL_IDENTITY_FINGERPRINT,
                "wakeEventId": null,
                "receiptEventId": null,
            },
            "strictMismatch": false,
...
fn identity_audit_basis(
    ...
    if agent == "local" && source == "local" {
        return IdentityAuditBasis {
            source: "local".to_string(),
            tool: None,
            registry_fingerprint: LOCAL_IDENTITY_FINGERPRINT.to_string(),
            wake_event_id: None,
...
pub fn audit_report_identity(
    ...
    if agent == "local" {
        return Ok(local_identity_audit(decl));
    }
