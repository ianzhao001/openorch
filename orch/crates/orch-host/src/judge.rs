//! 咨询综合（r56/B167，H30）：judge 提示词构造与五段结构化解析
//! （共识 / 矛盾 / 部分覆盖 / 独特洞察 / 盲点）。
//! judge 只产对比、不产终稿——终稿裁决权在 planner（design/04 committee 的 synthesizer 位）；
//! 因此不复用 preset.rs 的 quorum 投票语义。缺省 `--judge planner` 时本模块只落提示词与素材。
//! （planner 预置占位：lib.rs 声明先行入库，B167 在本文件内实现，勿动 lib.rs——frozenPaths。）
