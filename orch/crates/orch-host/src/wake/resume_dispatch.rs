//! Attempt-scoped replacement proof for a terminal implementation wake.
...
/// Durable fields copied from the owning `ResumeWakeLaunching` generation
/// into the replacement `WakeIssued`.
...
/// Authorize one changed-digest implementation wake only as a replacement for
/// the exact accepted and terminal source owned by the current resume action.
pub(super) fn authorize(
    events: &[EventRecord],
    active: &[super::ActiveWake],
    round: &str,
    agent: &str,
    continuation_id: &str,
    ...
    if !continuation_id.starts_with("implementation:") {
        bail!("resume replacement requires an implementation continuation");
    }
    let candidates = active
        .iter()
        .filter(|wake| wake.agent == agent && wake.continuation_id == continuation_id)
        .collect::<Vec<_>>();
    if candidates.len() != 1 {
        bail!(
            "resume replacement requires one exact active source, found {}",
            candidates.len()
        );
    }
    let source_wake_id = candidates[0].wake_id.clone();
    let (source_index, _source_event, source) = exact_source(events, round, &source_wake_id)?;
    if source.agent != agent || source.continuation_id != continuation_id {
        bail!("resume replacement source continuation identity drifted");
    }
    let (termination_index, evidence) =
        exact_terminal_evidence(events, round, source_index, &source)?;
    let (issued_index, _, _) = exact_resume_control(events, round, &source, scope, events.len())?;
    if termination_index >= issued_index {
        bail!("resume action must follow the source terminal proof");
    }
    match super::reissue_decision(
        active,
        agent,
        &super::WakeIntent::Continuation(continuation_id.to_string()),
    ...
    {
        super::ReissueOutcome::Reissue { superseded_wake_id }
            if superseded_wake_id == source_wake_id =>
        {
            Ok(superseded_wake_id)
        }
        super::ReissueOutcome::Reissue { superseded_wake_id } => bail!(
            "resume replacement source drifted: expected={source_wake_id} actual={superseded_wake_id}"
        ),
        super::ReissueOutcome::Refused { reason } => {
            bail!("resume replacement refused: {reason}")
        }
    }
}
...
mod tests {
    ...
    fn active() -> Vec<super::super::ActiveWake> {
        vec![super::super::ActiveWake {
            agent: AGENT.to_string(),
            continuation_id: continuation(),
            wake_id: SOURCE.to_string(),
            request_message_sha256: OLD_DIGEST.to_string(),
    ...
    #[test]
    fn exact_terminal_control_authorizes_and_projects_one_replacement() {
    ...
    #[test]
    fn missing_terminal_scope_or_wrong_generation_fails_closed() {
        let (events, action, owner, generation) = fixture(false);
        let scope = Scope {
            action_id: &action,
            owner: &owner,
            generation: &generation,
        ...
        let (events, action, owner, _generation) = fixture(true);
        let wrong = Scope {
            action_id: &action,
            owner: &owner,
        ...
        payload["resumeOwner"] = serde_json::json!(owner);
        payload["resumeLeaseGeneration"] = serde_json::json!(generation);
        payload["attemptId"] = serde_json::json!("wrong-attempt");
        events.push(successor);
        assert!(project(&events, ROUND).is_err());
    }
}
